...
(function () {
  ...
  function initModal() {
    ...
    // Enrollment form submits natively to Netlify (no e.preventDefault).
    // After submission, Netlify redirects to /?submitted=true
    // where checkFormSuccess() shows the success toast.
  }
  ...
  /* ─── Netlify Form Success Detection ─────────────────── */
  function checkFormSuccess() {
    if (!window.location.search.includes('submitted=true')) return;

    // Clean the URL without triggering a page reload
    const cleanUrl = window.location.pathname;
    window.history.replaceState(null, '', cleanUrl);

    // Show success toast after a brief delay so the page has rendered
    setTimeout(() => {
      showToast(
        '🎉 Registration Submitted!',
        'Thanks for enrolling! We\'ll be in touch within 24 hours.',
        'success'
      );
    }, 400);
  }
